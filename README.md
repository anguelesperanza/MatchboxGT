...
**Tile size:** every tilemap and collision helper takes an optional trailing
`tile_px` (default `16`; pass `8` for 8×8, or any power of two in 4..64). At 8px the
tileset is a **16-wide** grid (tile *N* at `((N&15)*8, tile_gy+(N>>4)*8)`) and a
128px screen holds 16 columns. Use the **same `tile_px`** for drawing and for the
collision helpers so the pixel↔tile math agrees.
...
| `odin run examples/tiles8/make_tiles8` | `examples/tiles8/tiles8.gtg.deflate` (8×8 floor/wall/accent + 16px hero) |
...
| `tiles8`  | the same idea at 8×8 tiles — `draw_tilemap(..., tile_px = 8)` fills a denser 16×12 room (`make_tiles8`) |
